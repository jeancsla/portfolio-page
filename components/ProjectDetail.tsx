...
const ProjectDetail: React.FC<ProjectDetailProps> = ({ project, t, onBack }) => {
  ...
  const renderBlock = (block: ContentBlock, index: number) => {
    switch (block.type) {
      case 'heading':
        return (
          <h2 key={index} className={`text-2xl font-bold mt-12 mb-6 ${isDark ? 'text-white' : 'text-slate-900'}`}>
            {block.text}
          </h2>
        );
      case 'paragraph':
        return (
          <p key={index} className={`text-lg leading-relaxed mb-6 ${isDark ? 'text-white/80' : 'text-slate-700'}`}>
            {block.text}
          </p>
        );
      case 'image':
        return (
          <figure key={index} className="my-10 group">
            <div className={`rounded-2xl overflow-hidden border p-2 transition-transform duration-500 group-hover:scale-[1.01] ${isDark ? 'border-white/10 bg-white/5' : 'border-black/5 bg-black/5'}`}>
              <img src={block.url} alt={block.alt} className="w-full h-auto rounded-xl object-cover max-h-[500px]" />
            </div>
            {block.text && (
              <figcaption className={`mt-4 text-sm italic text-center ${isDark ? 'text-white/50' : 'text-slate-500'}`}>
                {block.text}
              </figcaption>
            )}
          </figure>
        ...
        return (
          <ul key={index} className="space-y-3 mb-8 ml-4">
            {block.items?.map((item, i) => (
              <li key={i} className={`flex items-start gap-3 text-lg ${isDark ? 'text-white/80' : 'text-slate-700'}`}>
                <span className={`mt-2.5 w-1.5 h-1.5 rounded-full flex-shrink-0 ${isDark ? 'bg-indigo-400' : 'bg-indigo-600'}`}></span>
                {item}
              </li>
            ))}
          </ul>
      ...
      <div className={`glass p-6 sm:p-8 md:p-16 rounded-[2.5rem] ${isDark ? 'border-white/10' : 'border-white/20'}`}>
        ...
        <div className="mb-12">
          <div className="flex flex-wrap gap-2 mb-6">
            <span className={`text-[10px] font-bold uppercase tracking-[0.2em] px-3 py-1 rounded-full glass ${isDark ? 'text-indigo-300' : 'text-indigo-600'}`}>
              {project.date}
            </span>
            {project.tags?.map(tag => (
              <span key={tag} className={`text-[10px] font-bold uppercase tracking-[0.2em] px-3 py-1 rounded-full border ${isDark ? 'border-white/10 text-white/40' : 'border-black/10 text-black/40'}`}>
                #{tag}
              </span>
            ))}
          </div>
        ...
        <div className="project-content">
          {project.fullContent?.map((block, index) => renderBlock(block, index))}
        </div>

        {/* Optional GitHub Section */}
        {project.github && project.showReadme && (
          <div className={`mt-16 pt-16 border-t ${isDark ? 'border-white/10' : 'border-black/5'}`}>
            <div className="flex items-center justify-between mb-8">
              <h2 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Repository Details</h2>
              <a 
            ...
            {loading ? (
              ...
              <div className={`prose prose-sm max-w-none font-mono p-8 rounded-3xl ${isDark ? 'bg-black/30 text-white/60' : 'bg-white/40 text-slate-700'}`}>
